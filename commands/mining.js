...
module.exports = {
    name: 'mining',
    description: 'Đào đá để kiếm coins - yêu cầu ấn nút',
    async execute(message, args, client) {
        ...
        const miningMessage = await message.reply({
            embeds: [embed],
            components: [row]
        });

        // Collector cho button clicks
        const collector = miningMessage.createMessageComponentCollector({
        ...
        collector.on('collect', async (interaction) => {
            ...
            if (currentHits >= requiredHits) {
                ...
                // Giảm độ bền cuốc với random damage (1-3 điểm)
